...
export function useClaims() {
  ...
  function nextPage() { setPage(p => (hasNext ? p + 1 : p)) }
  function prevPage() { setPage(p => (canPrev ? p - 1 : p)) }
  ...
  return {
    ...
    status,
    setStatus: (v: 'All' | ClaimStatus) => { setStatus(v); resetPage() },
    ...
    page,
    pageSize,
    setPageSize: (n: number) => { setPageSize(n); resetPage() },
